...
function on(ee, evt, frequencies) {
    //if (Hertzy.WARNING) {
...
function off(ee, evt, frequencies) {
    //if (Hertzy.WARNING) {
    //  resetFrequenciesListeners(frequencies)
    //} else {
    const evtCount = ee.listenerCount(evt) - 1
...
function makeUpdateListeners(ee, frequencies) {
    return function updateListeners(evt, type) {
        if (Hertzy.WARNING) {
            ...
            switch (type) {
                case actions.ON:
                    on(ee, evt, frequencies)
                    break
                case actions.OFF:
                    off(ee, evt, frequencies)
                    break
                default:
...
function makeFrequencyClass(frequency, frequencies) {
...
function makeFrequency(frequency) {
    const Frequency = makeFrequencyClass(frequency, _frequencies)
