        ...
        switch (type) {
            case actions.ON:
                if (_eeCounter.has(evt)) {
                    const evtNextNr = _eeCounter.get(evt) + 1
                    if (evtNextNr > _ee.getMaxListeners() &&
                        evtNextNr > Math.max(_eeCounter.values())) {
                        _ee.setMaxListeners(evtNextNr)
                    }
                    _eeCounter.set(evt, evtNextNr)
                } else {
            ...
            case actions.OFF:
                if (_eeCounter.has(evt)) {
                    const evtPrevNr = _eeCounter.get(evt) - 1
                    if (evtPrevNr > 0) {
                        if (evtPrevNr > _ee.getMaxListeners() &&
                            evtPrevNr > Math.max(_eeCounter.values())) {
                            _ee.setMaxListeners(evtPrevNr)
                        }
                        _eeCounter.set(evt, evtPrevNr)
                    } else {
                        _eeCounter.delete(evt)
                    }
                }
                break
            default:
